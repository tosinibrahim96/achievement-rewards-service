...
final readonly class FakeTransferEffectRegistry
{
    public function __construct(
        ...
        $this->guardNamespace($this->environment, 'application environment');
        $this->guardNamespace($this->namespace, 'fake transfer effect namespace');
    }

    public function findForRequest(CashbackTransferRequest $request): ?CashbackTransferResult
    {
        $record = $this->read($request->providerReference);

        if ($record === null) {
            return null;
        }

        $fingerprint = $record['request_fingerprint'] ?? null;

        if (! is_string($fingerprint) || ! hash_equals($fingerprint, $this->fingerprint($request))) {
    ...
    {
        $record = $this->read($providerReference);

        return $record === null ? null : $this->resultFrom($record);
    }
    ...
    public function create(
        ...
        $record = [
            ...
            'status' => $status->value,
            'transfer_code' => 'TRF_FAKE_'.hash('sha256', $request->providerReference),
        ];
    ...
    private function read(string $providerReference): ?array
    {
    ...
    private function resultFrom(array $record): CashbackTransferResult
    {
    ...
    private function fingerprint(CashbackTransferRequest $request): string
    {
    ...
    private function guardNamespace(string $value, string $description): void
    {
